...
def graph():
    ...
    ax.set_xlabel('Average Pauli weight', fontsize=12)
    ax.set_ylabel('Number of modes (qubits)', fontsize=12)
    ax.plot(N, JW, '*', label="JW mapping")
    ax.plot(N, BK, 'x', label="BK mapping")
    ax.plot(N, TT1, '.', label="TT mapping")
    ax.plot(N, TT, '+', label="TT alpha-beta mapping")
...
if __name__ == "__main__":
    ...
    # JW = []
    # TT = []
    # for nmodes in range(2, 40):
    #     tt = TernaryTree(2*nmodes)
    #     # print("jw",end=' : ')
    #     # JW = JW + [main(tt)]
    #     tt.build_alphabeta_tree(2*nmodes)
    #     print("tt", end=' : ')
    #     tt.update_branchnum(st_enumeration1(2*nmodes))
    #     # draw(tt)
    #     # print(tt.branches())
    #     TT = TT + [main(tt)]
    #     # tt.update_branchnum(range(1,4*nmodes + 1))
    #     # tt.to0vac()
    #     # TT2 = TT2 + [main(tt)]
    #     # print("bk", end=' : ')
