...
from ..Tree.__init__ import TernaryTree


def tree(operator: FermionOperator | MajoranaOperato | InteractionOperator,
                 n_qubits: int =None,
                 tt: TernaryTree = None):
...
def _tree_fermion_operator(operator, n_qubits):
    # Compute the number of qubits.
    N = count_qubits(operator)
    if n_qubits is None:
        n_qubits = N
